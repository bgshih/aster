...
class AttentionRecognitionModel(model.Model):
  ...
  def __init__(self,
    ...
    super(AttentionRecognitionModel, self).__init__(
      feature_extractor,
      is_training)
    self._predictors_list = predictors_list
    self._is_training = is_training

    if len(self._predictors_list) == 0:
  ...
  def predict(self, preprocessed_inputs, scope=None):
    ...
    predictions_list = [
      predictor.predict(feature_maps) for predictor in self._predictors_list
    ]
  ...
  def loss(self, predictions_dict, scope=None):
    with tf.variable_scope(scope, 'Loss', list(predictions_dict.values())):
      losses = [
        predictor.loss(predictions_dict) for predictor in self._predictors_list
      ]
  ...
  def postprocess(self, predictions_dict, scope=None):
    with tf.variable_scope(scope, 'Postprocess', list(predictions_dict.values())):
      recognitions_list = [
        predictor.postprocess(predictions_dict) for predictor in self._predictors_list
      ]
      recognition_text = [r['text'] for r in recognitions_list]
      recognition_scores = [r['scores'] for r in recognitions_list]
      aggregated_recognition_dict = self._aggregate_recognition_results(
        recognition_text, recognition_scores)
  ...
  def provide_groundtruth(self, groundtruth_text_list, scope=None):
    with tf.variable_scope(scope, 'ProvideGroundtruth', [groundtruth_text_list]):
      ...
      for predictor in self._predictors_list:
        predictor.provide_groundtruth(groundtruth_text)

  def _aggregate_recognition_results(text_list, scores_list, scope=None):
