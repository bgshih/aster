...
from rare.core import optimizer, optimizer_pb2
...
class LearningRateBuilderTest(tf.test.TestCase):
  ...
  def testBuildConstantLearningRate(self):
    ...
    learning_rate = optimizer._create_learning_rate(
  ...
  def testRaiseErrorOnEmptyLearningRate(self):
    ...
    with self.assertRaises(ValueError):
      optimizer._create_learning_rate(
          learning_rate_proto, global_summaries)
...
class OptimizerBuilderTest(tf.test.TestCase):
  ...
  def testBuildRMSPropOptimizer(self):
    ...
    optimizer_object = optimizer.build(optimizer_proto, global_summaries)
    self.assertTrue(isinstance(optimizer_object, tf.train.RMSPropOptimizer))
  ...
  def testBuildMomentumOptimizer(self):
    ...
    text_format.Merge(optimizer_text_proto, optimizer_proto)
    optimizer_object = optimizer.build(optimizer_proto, global_summaries)
    self.assertTrue(isinstance(optimizer_object, tf.train.MomentumOptimizer))
  ...
  def testBuildMovingAverageOptimizer(self):
    ...
    optimizer_object = optimizer.build(optimizer_proto, global_summaries)
    self.assertTrue(
        isinstance(optimizer_object, tf.contrib.opt.MovingAverageOptimizer))
  ...
  def testBuildEmptyOptimizer(self):
    ...
    text_format.Merge(optimizer_text_proto, optimizer_proto)
    with self.assertRaises(ValueError):
      optimizer.build(optimizer_proto, global_summaries)
