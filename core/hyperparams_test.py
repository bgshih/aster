...
class HyperparamsBuilderTest(tf.test.TestCase):
  ...
  def test_default_arg_scope_has_conv2d_op(self):
    ...
    scope = hyperparams.build(conv_hyperparams_proto, is_training=True)
    self.assertTrue(self._get_scope_key(layers.conv2d) in scope)
  ...
  def test_return_batch_norm_params_with_notrain_during_eval(self):
    ...
    conv_hyperparams_proto = hyperparams_pb2.Hyperparams()
    text_format.Merge(conv_hyperparams_text_proto, conv_hyperparams_proto)
    scope = hyperparams.build(conv_hyperparams_proto, is_training=False)
    conv_scope_arguments = list(scope.values())[0]
    self.assertEqual(conv_scope_arguments['normalizer_fn'], layers.batch_norm)
    batch_norm_params = conv_scope_arguments['normalizer_params']
    self.assertAlmostEqual(batch_norm_params['decay'], 0.7)
